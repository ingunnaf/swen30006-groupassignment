...
public class Whist extends CardGame {
	...
	private AbstractPlayer[] players = {null, null, null, null};
...
//private int[] scores = new int[nbPlayers];
	...
	private void createPlayers(Hand h0, Hand h1, Hand h2, Hand h3) {
		...
		players[0] = new HumanPlayer(h0);
		players[1] = new HumanPlayer(h1);
		players[2] = new HumanPlayer(h2);
		players[3] = new HumanPlayer(h3);
...
//	      hands[i].setVerso(true);
	...
	private Optional<Integer> playRound() {  // Returns winner, if any
		...
		for (int i = 0; i < nbStartCards; i++) {
			trick = new Hand(deck);
			selected = null;
			if (players[i].getType() == "human" ) {  // Select lead depending on player type
				players[i].getHand().setTouchEnabled(true); //HumanPlayer.getHand ( )
				setStatus("Player 0 double-click on card to lead.");
				while (null == selected) delay(100);
				// if we move this to the human player class we won't be able to set status of the game?
			} else {
				setStatusText("Player " + nextPlayer + " thinking...");
				delay(thinkingTime);
				selected = players[nextPlayer].returnMove(trick);
			}
			...
			for (int j = 1; j < nbPlayers; j++) {
				...
				if (players[i].getType() == "human" ) {
					...
					players[i].getHand().setTouchEnabled(true); //HumanPlayer
					setStatus("Player 0 double-click on card to follow.");
					while (null == selected) delay(100);
				} else {
					setStatusText("Player " + nextPlayer + " thinking...");
					delay(thinkingTime);
					selected = players[nextPlayer].returnMove(trick); //npcs are playing: selected = Player[nextPlayer].returnSelected()
				}
